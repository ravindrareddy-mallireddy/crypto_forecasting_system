...
def compute_intercoin_correlation():
    ...
    pivot = (
        df.pivot(index="date", columns="symbol", values="returns")
        .dropna(how="all")
    )
...
if eda_option == "Summary Statistics":
    ...
    st.plotly_chart(
        px.histogram(returns_df, x="returns", nbins=50),
        use_container_width=True,
    )
    ...
    if ref_coin not in corr.columns:
        ...
        if not negative_corrs.empty:
            ...
            negative_title = f"Top 4 Negatively Correlated with {ref_coin}"
            caption_text = (
                "Negative correlations indicate divergent price movement "
                "relative to the selected cryptocurrency."
            )
        else:
            bottom_set = (
                coin_corr
            ...
            negative_title = f"Top 4 Least Correlated with {ref_coin}"
            caption_text = (
                "No strong negative correlations were observed. "
                "Least correlated cryptocurrencies are shown instead, "
                "reflecting the generally positive co-movement in crypto markets."
            )
    ...
    if out_df.empty:
        ...
        st.dataframe(out_df)
        st.plotly_chart(px.scatter(out_df, x="date", y="close"), use_container_width=True)

elif eda_option == "Missing Data":
    st.dataframe(pd.read_csv(EDA_DIR / "missing" / "missing_summary.csv"))

elif eda_option == "Volume Analysis":
    df = pd.read_csv(EDA_DIR / "volume" / f"{symbol}_volume_stats.csv", parse_dates=["date"])
    ...
    st.plotly_chart(px.line(df, x="date", y="returns_squared"), use_container_width=True)
